<?php
Class Insurants extends CI_Model {
   ...
   public function getAllInsurants(){
	   $sql = 'SELECT * 
				 FROM POJ_T_USER
   ...
   public function fetchInsurantss($limit, $offset, $orderBy, $orientation){
		if ($offset==''){
			$offset=0;
		} 	
		switch ($orderBy){
   ...
   public function getCountInsurants(){
   ...
   public function getInsurantName($user_id){
   ...
   public function getInsurantDetailInfo($user_id){
		$sql = 'SELECT * 
				 FROM POJ_T_USER
				 LEFT JOIN POJ_T_USER_ADRESS ON POJ_T_USER.USER_ADRESS_ID = POJ_T_USER_ADRESS.ADRESS_ID
				 LEFT JOIN POJ_T_USER_HIRE ON POJ_T_USER.USER_HIRE_ID = POJ_T_USER_HIRE.HIRE_ID
   ...
   public function getInsurantIdByPIN($user_pin){
   ...
   public function getInsurantInfoByPIN($user_pin){
	   $user_pin_formated = substr($user_pin, 0, 6).'/'.substr($user_pin, 6, 4);
   ...
   public function getInsurantAdressByPIN($user_pin){
		$user_pin_formated = substr($user_pin, 0, 6).'/'.substr($user_pin, 6, 4);
		$sql = 'SELECT *
				FROM POJ_T_USER
   ...
   public function getInsurantBankByPIN($user_pin){
		$user_pin_formated = substr($user_pin, 0, 6).'/'.substr($user_pin, 6, 4);
		$sql = 'SELECT *
				FROM POJ_T_USER
				LEFT JOIN POJ_T_USER_BANK ON POJ_T_USER.USER_BANK_ID=POJ_T_USER_BANK.BANK_ID
   ...
   public function getInsurantStateByPIN($user_pin){
		$user_pin_formated = substr($user_pin, 0, 6).'/'.substr($user_pin, 6, 4);
		$sql = 'SELECT *
				FROM POJ_T_USER
				LEFT JOIN POJ_T_USER_ADRESS ON USER_ADRESS_ID=ADRESS_ID
   ...
   public function findSpecificInsurant($string) {
   ...
   public function canIModifyInsurant($user_id) {
   ...
   public function fetchInsurants($limit, $offset, $orderBy, $orientation){
		if ($offset==''){
			$offset=0;
		} 	
		switch ($orderBy){
