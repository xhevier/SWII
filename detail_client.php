...
if ($this->session->userdata('permisson')==0 OR $this->session->userdata('permisson')==1) {
	...
	<div id="dialogEmployeePDF" class="dialogEmployee">
		...
		<div style=" text-align:center;">
			<div class="button-group" style="padding-top: 15px;">
				...
				<a href="<? echo base_url('Admin/acceptKlientPDF/'.$user->USER_ID); ?>" class="button">Udáje sú prekontrolované a chcem vygenerovať zmluvu</a>
			</div>
		</div>
	</div>
	...
	<div id="dialogEmployeeStorno" class="dialogEmployee">
			...
			<div style=" text-align:center;">
				<div class="button-group" style="padding-top: 12px;">
					...
					<a href="<? echo base_url('Admin/stornoClient/'.$user->USER_ID); ?>" class="button">Áno, chcem stornovať klienta</a>
				</div>
			</div>
		</div>
		...
		<div id="dialogEmployeeFire" class="dialogEmployee">
			...
			<div style=" text-align:center;">
				<div class="button-group" style="padding-top: 15px;">
					...
					<a href="<? echo base_url('Admin/fireClient/'.$user->USER_ID); ?>" class="button">Áno, chcem deaktivovať klienta</a>
				</div>
			</div>
		</div>
						...
						if ($counter<10){
							...
							if ($agreement->AGREEMENT_TYPE=='L'){
								if ($agreement->AGREEMENT_PLATNA=='A'){
									echo '<a href="'.base_url('Admin/redirectEditLifeAgreement/'.$agreement->AGREEMENT_ID).'/1">'.$agreement->AGREEMENT_CODE.'</a>'; ?>
									<a href="<?php echo base_url('Admin/redirectEditLifeAgreement/'.$agreement->AGREEMENT_ID.'/1') ?>">
										<img border="0" width="80" style="margin-left:12px; margin-top:5px" src="<?php echo base_url("images/form.png")?>">
									</a> <?php
								} else {
									echo '<a href="'.base_url('Admin/redirectEditLifeAgreement/'.$agreement->AGREEMENT_ID).'/1"><span style="color:red">'.$agreement->AGREEMENT_CODE.'</span></a>'; ?>
									<a href="<?php echo base_url('Admin/redirectEditLifeAgreement/'.$agreement->AGREEMENT_ID.'/1') ?>">
										<img border="0" width="80" style="margin-left:12px; margin-top:5px" src="<?php echo base_url("images/form.png")?>">
									</a> <?php
								}								
								 ...
								 if ($agreement->AGREEMENT_PLATNA=='A'){
									 echo '<a href="'.base_url('Admin/redirectEditTravelAgreement/'.$agreement->AGREEMENT_ID).'/1">'.$agreement->AGREEMENT_CODE.'</a>'; ?>
									<a href="<?php echo base_url('Admin/redirectEditTravelAgreement/'.$agreement->AGREEMENT_ID.'/1') ?>">
										<img border="0" width="80" style="margin-left:12px; margin-top:5px" src="<?php echo base_url("images/form.png")?>">
									</a> <?php
								 } else {
									 echo '<a href="'.base_url('Admin/redirectEditTravelAgreement/'.$agreement->AGREEMENT_ID).'/1"><span style="color:red">'.$agreement->AGREEMENT_CODE.'</a>'; ?>
									<a href="<?php echo base_url('Admin/redirectEditTravelAgreement/'.$agreement->AGREEMENT_ID.'/1') ?>">
										<img border="0" width="80" style="margin-left:12px; margin-top:5px" src="<?php echo base_url("images/form.png")?>">
									</a> <?php 
								 }
    ...
    <div class="div_lane" style="">
		...
		<a href="<?php echo base_url('Admin/redirectAllClients/1/1')?>" class="button">Zpět</a> 
		<?php if ($user->USER_PLATNA=='A'){
			...
			?> <a onclick="alertShowFire();" class="button">Deaktivovať</a> <?php		
				if ($user->USER_ACCEPT_PDF=='N'){
					echo '<a href="'.base_url('Admin/redirectModifyClient/'.$user->USER_ID).'" class="button">Upravit</a>';
				}
			}								
			?>					
        </div>
