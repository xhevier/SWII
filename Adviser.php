...
class Adviser extends CI_Controller {
	...
	public function redirectDefaultUser(){
		$this->load->view('adviser/users/default_user');
	 }
	
	public function redirectFindUser(){
		$data['input']='';
		$data['searchEmployeeError'] = '';
        $this->load->view('adviser/users/find_user',$data);
    }
	...
	public function redirectDetailUser($user_id){
        ...
        $this->load->view('adviser/users/detail_user',$data);
    }
	
	public function acceptEmployeePDF($user_id){
		$this->load->model('Users');
		$this->Users->acceptEmployeePDF($user_id);
		redirect (base_url('Adviser/redirectDetailUser/'.$user_id));
	}
	
	public function acceptKlientPDF($user_id){
		$this->load->model('Users');
		$this->Users->acceptEmployeePDF($user_id);
		redirect (base_url('Adviser/redirectDetailClient/'.$user_id));
	}
	...
	public function redirectNewUser(){	
		...
		$this->load->view('adviser/users/new_user',$data);
    }
	...
	/* validation $data['newEmployeeIban']*/
		...
		if (IsEmpty($data['newEmployeeErrors'])) {
			...
			redirect (base_url('Adviser/redirectDetailUser/'.$new_user_id));
		} else {
			$this->load->view('adviser/users/new_user',$data);
		}
	}
	...
	public function redirectModifyEmployee($user_id){
        ...
        $this->load->view('adviser/users/modify_user',$data);
	}
	...
	public function redirectAllUsers($orderBy, $orientation){
        ...
        $configuration=$this->getConfigurationForPagination($pageLimit, base_url('Adviser/redirectAllUsers/'.$orderBy.'/'.$orientation.'/'), $totalRows, 5);	 
		$this->pagination->initialize($configuration);
		$this->load->view('adviser/users/show_all_users',$data);
    }
	...
	public function findEmployee(){
		...
		if (strlen($this->input->post('findEmployeeText'))<2){
			...
			if ($query->num_rows() == 0 ) {
				...
				redirect(base_url('Adviser/redirectAllUsers/1/1'));
			}		
		}
	}
	...
	public function clearEmployeeFilter($orderBy,$orientation){
		...
		redirect(base_url('Adviser/redirectAllUsers/'.$orderBy.'/'.$orientation));
	}     
	...
	public function redirectDefaultClient(){
		$this->load->view('adviser/clients/default_client');
	 }
	 
	public function redirectFindClient(){
		$data['input']='';
		$data['searchClientError'] = '';
        $this->load->view('adviser/clients/find_client',$data);
    }
	...
	public function redirectDetailClient($user_id){
		...
		$data['user'] = $this->Users->getUserDetailInfo($user_id);
		$data['agreements'] = $this->Agreements->getAllUserAgreements($user_id);
        $this->load->view('adviser/clients/detail_client',$data);
    }
	...
	public function redirectNewClient(){	
		...
		$data['newEmployeeBankName'] = '';
		$data['newEmployeeIban'] = '';
		$this->load->view('adviser/clients/new_client',$data);
    }
	...
	public function redirectAllClients($orderBy, $orientation){
        ...
        $configuration=$this->getConfigurationForPagination($pageLimit, base_url('Adviser/redirectAllClients/'.$orderBy.'/'.$orientation.'/'), $totalRows, 5);	 
		$this->pagination->initialize($configuration);
		$this->load->view('adviser/clients/show_all_clients',$data);
    }
	...
	public function redirectModifyClient($user_id){
		...
		$data['BANK_IBAN'] = $user->BANK_IBAN;
		$data['ADRESS_STATE'] = $user->ADRESS_STATE;
        $this->load->view('adviser/clients/modify_client',$data);
	}
	...
	public function findClient(){
		...
		if (strlen($this->input->post('findClientText'))<2){
			...
			if ($query->num_rows() == 0 ) {
				...
				$query = $query->row();
				redirect(base_url('Adviser/redirectDetailClient/'.$query->USER_ID));
			} else {
				$query = $query->row();
				...
				redirect(base_url('Adviser/redirectAllClients/1/1'));
			}		
		}
	}
	...
	public function clearClientFilter($orderBy,$orientation){
		...
		redirect(base_url('Adviser/redirectAllClients/'.$orderBy.'/'.$orientation));
	} 
	...
	/* validation $data['newEmployeeIban']*/
		...
		if (IsEmpty($data['newEmployeeErrors'])) {
			$this->load->model('Clients');
			$new_user_id = $this->Users->insertNewUser();
			redirect (base_url('Adviser/redirectDetailClient/'.$new_user_id));
		} else {
			$this->load->view('adviser/clients/new_client',$data);
		}
	}
	...
	public function redirectDefaultAgreement(){
		$this->load->view('adviser/agreements/default_agreement');
	}
	
	public function redirectFindAgreement(){
		$data['input']='';
		$data['searchEmployeeError'] = '';
		$this->load->view('adviser/agreements/find_agreement',$data);
	}
	...
	public function redirectAllAgreements($order_by, $orientation){
		...
		$configuration=$this->getConfigurationForPagination($pageLimit, base_url('Adviser/redirectAllAgreements/'.$order_by.'/'.$orientation), $totalRows, 5);
		$this->pagination->initialize($configuration);
		$this->load->view('adviser/agreements/all_agreements',$data);
	}
  
	public function redirectNewAgreements(){
        $this->load->view('adviser/agreements/new_agreement_sports');
    }
	...
	public function findAgreement(){
		...
		if (strlen($this->input->post('findAgreementText'))<2){
			...
			if ($query->num_rows() == 0 ) {
				...
				redirect(base_url('Adviser/redirectAllAgreements/1/1'));
			}		
		}
	}
    ...
    $data['newAgreementUser'] = array ('NewNumber' =>'' ,'newPIN' => '','errorMessage' => '','errorMessagePrice' => '','errorMessageDateTo' => '','userName' => '','userBirth' => '','userAge' => '','userAdress' => '','userState' => '','userBank' => '','userPrice' => '','userCurrency' => '','userDateTo' => '','userFrequency' => '');
		$this->load->view('adviser/agreements/agreement_extension', $data);
	}

	public function redirectCancelAgreement(){
    $data['newAgreementUser'] = array ('NewNumber' =>'' ,'newPIN' => '','errorMessage' => '','errorMessagePrice' => '','errorMessageDateTo' => '','userName' => '','userBirth' => '','userAge' => '','userAdress' => '','userState' => '','userBank' => '','userPrice' => '','userCurrency' => '','userDateTo' => '','userFrequency' => '', '');
		$this->load->view('adviser/agreements/agreement_cancel', $data);
	}*/
	...
	public function redirectChooseNewAgreement($type){	
		...
		switch ($type){
			case 0:	
				$this->load->view('adviser/agreements/choose_new_agreement');
			break;
			case 1: 
				...
				$this->load->view('adviser/agreements/new_agreement_life',$data);
			break;
			case 2:
				...
				$this->load->view('adviser/agreements/new_agreement_travel',$data);
			break;
			case 3: 
				...
				$this->load->view('adviser/agreements/new_agreement_pension',$data);
			break;
			case 4: 
				...
				$this->load->view('adviser/agreements/new_agreement_sports',$data);
			break;
		}	
	}
...
/* Zivotne poistenie */
	  public function redirectEditLifeAgreement($agreement_id){
		...
		$this->load->view('adviser/agreements/edit_agreement_life', $data);
	  }
		...
		if ($this->input->post('newLifeTrace') == "Dohledat"){
			...
			if ($this->input->post('newLifeName')!=''){
				...
				if (($data['newAgreementUser']['errorMessagePrice']=='')AND($data['newAgreementUser']['errorMessage']=='')AND($data['newAgreementUser']['errorMessageDateTo']=='')){												
					...
					redirect(base_url('Adviser/redirectEditLifeAgreement/'.$agreement_id));
				} else {
					$this->load->view('adviser/agreements/edit_agreement_life',$data);
				}	
			} else {
...
/* cestovne poistenie */
	public function acceptTravelAgreementPDF($agreement_id){
		...
		redirect(base_url('Adviser/redirectEditTravelAgreement/'.$agreement_id));
	}
	...
	public function redirectEditTravelAgreement($agreement_id){
		...
		$this->load->view('adviser/agreements/edit_agreement_travel', $data);
	}
	...
	public function editTravelAgreement($agreement_id){
		...
		$user_id = $this->Users->getUserIdByPIN($this->input->post('newTravelPIN'));
				...
				if (($data['newAgreementUser']['errorMessageCurrency']=='') AND ($data['newAgreementUser']['errorMessageDate']=='')){
					...
					redirect(base_url('Adviser/redirectEditTravelAgreement/'.$this->input->post('newTravelId')));
				} else {
					$this->load->view('adviser/agreements/edit_agreement_travel',$data);
				}
	}
	...
	public function redirectDefaultInvoice(){
		$this->load->view('adviser/invoices/default_invoice');
	}
	
	public function redirectFindInvoice(){
		$data['input']='';
		$data['searchBillError'] = '';
        $this->load->view('adviser/invoices/find_bill',$data);
    }
	
	public function clearBillsFilter($orderBy,$orientation){
		$this->session->set_userdata('billsDefaultWhere', '(1=1)');
		redirect(base_url('Adviser/redirectAllInvoices/'.$orderBy.'/'.$orientation));
	}
	...
	public function findInvoice(){
		...
		if (strlen($this->input->post('findBillText'))<1){
			...
			if ($query->num_rows() == 0 ) {
				...
				redirect(base_url('Adviser/redirectAllInvoices/1/1'));
			}		
		}
	}
	...
	public function redirectAllInvoices($orderBy, $orientation){
        ...
        $configuration=$this->getConfigurationForPagination($pageLimit, base_url('Adviser/redirectAllInvoices/'.$orderBy.'/'.$orientation.'/'), $totalRows, 5);	 
		$this->pagination->initialize($configuration);
		$this->load->view('adviser/invoices/all_invoices',$data);
    }
  ...
  public function redirectSettings(){
    ...
    $this->load->view('adviser/settings/settings',$data);   
  }
